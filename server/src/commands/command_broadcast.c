...
static int shortest_distance(int a, int b, int world_size)
{
    ...
    if (abs(direct) <= abs(wrap_positive) &&
        abs(direct) <= abs(wrap_negative)) {
        return direct;
...
{
    delta->dx = shortest_distance(receiver->x, emitter->x,
        server->map_data->width);
    delta->dy = shortest_distance(receiver->y, emitter->y,
...
static int compute_sound_direction(int dx, int dy, int receiver_orientation)
{
    ...
    offset = get_direction_offset(receiver_orientation);
    direction = ((sector + offset) % 8) + 1;
    return direction;
}
...
static void send_broadcast_to_player(server_t *server,
    client_connection_t *from, client_connection_t *to, const char *message)
{
    int direction = get_broadcast_direction(server,
        from->client.player, to->client.player);

    server_send_to_client(to, "message %d, %s\n", direction, message);
    printf("Broadcast from player %d to player %d: direction %d, "
        "message: %s\n", from->client.player->id, to->client.player->id,
        direction, message);
}
    ...
    client_connection_t *sender, const char *message)
{
    ...
    for (current = server->clients; current; current = current->next) {
        if (current->client.type == AI_CLIENT &&
            current->client.player && current != sender) {
            send_broadcast_to_player(server, sender, current, message);
        }
    }
}

static char *extract_clean_message(const char *command)
{
    ...
    clean_message = strdup(message);
    if (clean_message) {
        newline = strchr(clean_message, '\n');
        if (newline) {
            *newline = '\0';
        }
    }
    ...
    const char *command)
{
    ...
    clean_message = extract_clean_message(command);
    printf("Player %d broadcasting: %s\n",
        client->client.player->id,
        clean_message ? clean_message : command);
